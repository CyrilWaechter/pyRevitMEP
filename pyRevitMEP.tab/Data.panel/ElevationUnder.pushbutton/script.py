...
def get_element_origin(element):
    ...
    try:
        ...
        return (
            ...
            + element.Location.Curve.GetEndPoint(1)
        ) / 2
...
def compute_elevation(view3d, parameter_name):
    ...
    with revit.Transaction("ComputeElevation", doc=doc):
        for id in uidoc.Selection.GetElementIds():  # type: ElementId
            element = doc.GetElement(id)
            origin = get_element_origin(element)
            direction = XYZ.BasisZ
            try:
                reference = reference_intersector.FindNearest(
                    origin, direction
                ).GetReference()
            except AttributeError:
                logger.info(
                    "Failed to find a roof or a floor above Element {} {}, id {}".format(
                        get_type_name(element), element.Name, element.Id
                    )
                )
                continue
